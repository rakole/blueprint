...
Final release validation at `a37deccf2bf151e869688527903ef7b31f9e3753` passed typechecking, the regenerated build, and all 2,556 canonical tests with no failures or skips. Shipped-only MCP probes passed on Linux Node 20.20.2 and macOS Node 26.0.0. Gemini installation and disposable clean-home checks passed; authenticated Gemini prompts and Tabnine installation were explicitly skipped. The separate [release validation receipt](portable-map-release-validation.json) records the checks, review fixes, unchanged dependency advisories, and post-review 10,000-file Linux fixture. That fixture published and retained two readable generations, with 451 bounded packets; it did not walk the full persisted public receipt lifecycle at 10,000 files or establish macOS large-map latency. These repository checks do not turn the hosted evaluation into a successful benchmark.
