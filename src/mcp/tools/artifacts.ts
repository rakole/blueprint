...
import { validateCodebaseContent } from "../codebase-authoring.js";
...
export const CODEBASE_PUBLICATION_PATH = `${BLUEPRINT_CODEBASE_PATH}/.publication.json`;
...
const BLUEPRINT_REPO_LOCK_OWNER_FILE = "owner";
const BLUEPRINT_REPO_LOCK_LEASE_FILE = "lease";
const BLUEPRINT_REPO_LOCK_RECOVERY_GUARD_PREFIX = "owner.";
const BLUEPRINT_REPO_LOCK_RETRY_MS = 50;
const BLUEPRINT_REPO_LOCK_STALE_MS = 60_000;
...
function normalizeBlueprintArtifactPathForClassification(relativePath: string): string {
  return relativePath.replace(/\\/g, "/").replace(/\/+$/g, "");
}

function isOperationalOnlyBlueprintArtifact(relativePath: string): boolean {
  const artifact = normalizeBlueprintArtifactPathForClassification(relativePath);
  const blueprintPrefix = `${BLUEPRINT_DIR}/`;

  if (artifact === `${BLUEPRINT_DIR}/mcp-write-failures.ndjson`) {
    return true;
  }

  if (!artifact.startsWith(blueprintPrefix)) {
    return false;
  }

  const blueprintLocalPath = artifact.slice(blueprintPrefix.length);
  ...
  return (
    /^\.?.+\.\d+\.\d+\.[a-z0-9]+\.tmp$/i.test(artifactName) ||
    /^.+\.tmp-\d+-\d+$/i.test(artifactName)
  );
}
...
type BlueprintRepoLockRecoveryGuardHandle = {
  lockPath: string;
  recoveryPath: string;
  ownerPath: string;
  token: string;
...
const RESEARCH_CONTRACT = readArtifactContract("phase.research");
const REQUIRED_RESEARCH_SECTIONS = RESEARCH_CONTRACT.requiredHeadings;
const RESEARCH_CANONICAL_HEADINGS = [
  ...REQUIRED_RESEARCH_SECTIONS, ...(RESEARCH_CONTRACT.modelContract?.renderedHeadings ?? []),
  "Confidence Breakdown", "Claim Support Ledger"
];
const RESEARCH_TEMPLATE_PLACEHOLDER_SIGNALS = RESEARCH_CONTRACT.placeholderSignals;
...
const defaultJsonFileSystem: BlueprintArtifactsJsonFileSystemForTest = {
...
let jsonFileSystemForTest: BlueprintArtifactsJsonFileSystemForTest | null = null;
let repoLockTimingForTest: BlueprintRepoLockTimingForTest | null = null;
let repoLockRecoveryHooksForTest: BlueprintRepoLockRecoveryHooksForTest | null = null;

function activeJsonFileSystem(): BlueprintArtifactsJsonFileSystemForTest {
...
export function blueprintRepoLockNameSegment(value: string): string {
  const normalized = value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
...
function blueprintRepoLockRetryMs(): number {
  return positiveTimingOverride(
    repoLockTimingForTest?.retryMs,
    BLUEPRINT_REPO_LOCK_RETRY_MS
  );
}

function blueprintRepoLockStaleMs(): number {
  return positiveTimingOverride(
    repoLockTimingForTest?.staleMs,
    BLUEPRINT_REPO_LOCK_STALE_MS
  );
}

function blueprintRepoLockHeartbeatMs(): number {
  return positiveTimingOverride(
    repoLockTimingForTest?.heartbeatMs,
    Math.max(25, Math.floor(blueprintRepoLockStaleMs() / 4))
  );
}

export const blueprintArtifactsTestHooks: {
  ...
  setJsonFileSystemForTest(fileSystem) {
    const previous = jsonFileSystemForTest;
    jsonFileSystemForTest = fileSystem;

    return () => {
      jsonFileSystemForTest = previous;
    };
  },
  setRepoLockTimingForTest(timing) {
    const previous = repoLockTimingForTest;
    repoLockTimingForTest = { ...timing };

    return () => {
      repoLockTimingForTest = previous;
    };
  },
  setRepoLockRecoveryHooksForTest(hooks) {
    const previous = repoLockRecoveryHooksForTest;
    repoLockRecoveryHooksForTest = { ...hooks };

    return () => {
      repoLockRecoveryHooksForTest = previous;
    };
  }
};
...
function blueprintRepoLockOwnerPath(lockPath: string): string {
  return path.join(lockPath, BLUEPRINT_REPO_LOCK_OWNER_FILE);
}

function blueprintRepoLockLeasePath(lockPath: string): string {
  return path.join(lockPath, BLUEPRINT_REPO_LOCK_LEASE_FILE);
}

function blueprintRepoLockRecoveryPath(lockPath: string): string {
  return `${lockPath}.recovery`;
}

function blueprintRepoLockRecoveryGuardOwnerPath(recoveryPath: string, token: string): string {
  return path.join(recoveryPath, `${BLUEPRINT_REPO_LOCK_RECOVERY_GUARD_PREFIX}${token}`);
}

function blueprintRepoLockQuarantinePath(lockPath: string): string {
  return path.join(
    path.dirname(lockPath),
    `.${path.basename(lockPath)}.${process.pid}.${Date.now()}.${Math.random()
      .toString(36)
      .slice(2)}.stale`
  );
}

async function writeBlueprintRepoLockFile(filePath: string, contents: string): Promise<void> {
  await fs.writeFile(filePath, `${contents}\n`, "utf8");
}

async function readBlueprintRepoLockOwner(
  lockHandle: BlueprintRepoLockHandle
): Promise<string | null> {
  return readBlueprintRepoLockOwnerAtPath(lockHandle.ownerPath);
}

async function readBlueprintRepoLockOwnerAtPath(ownerPath: string): Promise<string | null> {
  try {
...
async function refreshBlueprintRepoLockLease(
  lockHandle: BlueprintRepoLockHandle
): Promise<boolean> {
  const ownerToken = await readBlueprintRepoLockOwner(lockHandle);

  if (ownerToken !== lockHandle.token) {
    return false;
  }

  await writeBlueprintRepoLockFile(lockHandle.leasePath, lockHandle.token);
  return true;
}

async function getBlueprintRepoLockPathAgeMs(targetPath: string): Promise<number | null> {
  try {
...
async function getBlueprintRepoLockRecoveryGuardAgeMs(
  recoveryPath: string
): Promise<number | null> {
  let entries: string[];
  ...
  for (const entry of entries) {
    if (!entry.startsWith(BLUEPRINT_REPO_LOCK_RECOVERY_GUARD_PREFIX)) {
      continue;
    }
  ...
  return getBlueprintRepoLockPathAgeMs(recoveryPath);
}

async function getBlueprintRepoLockAgeMs(lockPath: string): Promise<number | null> {
  ...
  return getBlueprintRepoLockPathAgeMs(lockPath);
}

async function readBlueprintRepoLockOwnerForPath(lockPath: string): Promise<string | null> {
  return readBlueprintRepoLockOwnerAtPath(blueprintRepoLockOwnerPath(lockPath));
}

async function createBlueprintRepoLockRecoveryGuardHandle(
  lockPath: string,
  recoveryPath: string
): Promise<BlueprintRepoLockRecoveryGuardHandle> {
  const token = `${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  const recoveryGuard: BlueprintRepoLockRecoveryGuardHandle = {
    lockPath,
    recoveryPath,
    ownerPath: blueprintRepoLockRecoveryGuardOwnerPath(recoveryPath, token),
    token
  };
...
async function refreshOwnedBlueprintRepoLockRecoveryGuard(
  recoveryGuard: BlueprintRepoLockRecoveryGuardHandle
): Promise<boolean> {
  let stats;
  ...
  if (Date.now() - stats.mtimeMs > blueprintRepoLockStaleMs()) {
    return false;
  }
...
async function releaseBlueprintRepoLockRecoveryGuard(
  recoveryGuard: BlueprintRepoLockRecoveryGuardHandle
): Promise<void> {
  if (!(await refreshOwnedBlueprintRepoLockRecoveryGuard(recoveryGuard))) {
    return;
  }
...
async function createBlueprintRepoLockHandle(
  lockPath: string
): Promise<BlueprintRepoLockHandle> {
  const token = `${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  const lockHandle: BlueprintRepoLockHandle = {
    lockPath,
    ownerPath: blueprintRepoLockOwnerPath(lockPath),
    leasePath: blueprintRepoLockLeasePath(lockPath),
    token
  };
...
async function reclaimStaleBlueprintRepoLockRecoveryGuard(
  recoveryPath: string
): Promise<boolean> {
  const ageMs = await getBlueprintRepoLockRecoveryGuardAgeMs(recoveryPath);

  if (ageMs === null) {
    return true;
  }

  if (ageMs <= blueprintRepoLockStaleMs()) {
    return false;
  }
...
async function tryAcquireBlueprintRepoLockRecoveryGuard(
  lockPath: string
): Promise<BlueprintRepoLockRecoveryGuardHandle | null> {
  ...
  for (;;) {
    try {
      ...
      if (await reclaimStaleBlueprintRepoLockRecoveryGuard(recoveryPath)) {
        continue;
      }
...
async function recoverStaleBlueprintRepoLock(lockPath: string): Promise<boolean> {
  const recoveryGuard = await tryAcquireBlueprintRepoLockRecoveryGuard(lockPath);
  ...
  try {
    const ageMs = await getBlueprintRepoLockAgeMs(lockPath);
    ...
    const observedOwner = await readBlueprintRepoLockOwnerForPath(lockPath);

    await repoLockRecoveryHooksForTest?.beforeStaleLockQuarantine?.(lockPath);

    if (!(await refreshOwnedBlueprintRepoLockRecoveryGuard(recoveryGuard))) {
      return false;
    }

    const currentAgeMs = await getBlueprintRepoLockAgeMs(lockPath);

    if (currentAgeMs === null) {
      return true;
    }

    if (currentAgeMs <= blueprintRepoLockStaleMs()) {
      return false;
    }

    const currentOwner = await readBlueprintRepoLockOwnerForPath(lockPath);
    ...
    await releaseBlueprintRepoLockRecoveryGuard(recoveryGuard).catch(() => undefined);
    await repoLockRecoveryHooksForTest?.afterRecoveryGuardRelease?.(lockPath);
  }
}

async function acquireBlueprintRepoLock(lockPath: string): Promise<BlueprintRepoLockHandle> {
  const retryDelayMs = blueprintRepoLockRetryMs();
  ...
  for (;;) {
    try {
      ...
      try {
        ...
        if (ageMs !== null && ageMs > blueprintRepoLockStaleMs()) {
          await repoLockRecoveryHooksForTest?.beforeStaleRecoveryClaim?.(lockPath);

          if (await recoverStaleBlueprintRepoLock(lockPath)) {
            continue;
          }
        }
      } catch (statError) {
...
async function releaseBlueprintRepoLock(lockHandle: BlueprintRepoLockHandle): Promise<void> {
  const ownerToken = await readBlueprintRepoLockOwner(lockHandle);
  ...
  await fs.rm(lockHandle.ownerPath, { force: true }).catch(() => undefined);
  await fs.rm(lockHandle.leasePath, { force: true }).catch(() => undefined);
  await fs.rmdir(lockHandle.lockPath).catch(() => undefined);
}
...
export async function withBlueprintRepoLock<T>(
  ...
  const stopHeartbeat = startBlueprintRepoLockHeartbeat(lockHandle);
...
function splitResearchTypedReferenceIds(value: string): string[] {
...
function sourceRegisterRowHasConcreteEvidence(row: ResearchMarkdownRow): boolean {
  ...
  // Supplied references can be named interview notes or quoted material without a public URL.
  const suppliedLabel = /^supplied$/i.test(row.lane?.trim() ?? "") && reference.length > 0 &&
    !/^(?:none|null|undefined|n\/?a|unknown|unchecked|tbd|todo|<[^>]+>)$/i.test(reference);
...
export function canonicalizeResearchRequiredHeadings(
  ...
  const canonicalHeadingByKey = new Map(
    RESEARCH_CANONICAL_HEADINGS.map((heading) => [normalizeResearchHeadingKey(heading), heading] as const)
  );
  for (const [alias, canonical] of Object.entries({
    Overview: "Summary", "Executive Summary": "Summary", "Research Summary": "Summary",
    "Recommended Approach": "Recommendations", "Implementation Guidance": "Recommendations",
    References: "Sources", Evidence: "Sources"
...
function hasExplicitBlockedRecommendationProse(section: string): boolean {
...
export function researchHasPlanningBlockers(content: string): boolean {
  ...
  const plain = (value: string): string => value.replace(/[*_`]/g, "");
  return collectResearchRecommendationRows(visible).some((row) => /^blocked$/i.test(plain(row.status ?? "").trim()))
    || hasExplicitBlockedRecommendationProse(recommendations)
    || /(?:^|\n)\s*(?:[-+]\s*)?blocking\s*:/i.test(plain(questions))
    || parseResearchMarkdownTable(questions).some((row) => /^(?:true|yes|blocking)$/i.test(plain(row.blocking ?? row.status ?? "").trim()));
}
...
export function validateResearchArtifactContent(content: string): {
  ...
  const issue = (heading: string | undefined, code: string, message: string, repair: string): void => {
    issues.push(message);
  ...
  if (!/^#\s+\S[^\n]*\r?(?:\n|$)/.test(visible.trimStart())) issue(undefined, "research.title_missing",
    "Research needs a document title outside code examples.", "Start the research document with a descriptive H1 title.");
  if (visible.includes(SCAFFOLD_GENERATED_MARKER) || matchedDiscussionScaffoldRows(visible, RESEARCH_TEMPLATE_PLACEHOLDER_SIGNALS).length > 0) issue(undefined, "research.placeholder_present",
    "Research still contains unfilled scaffold placeholders.", "Replace unfilled placeholders with the actual findings or omit the optional section.");

  const headings = new Set(scanResearchMarkdown(normalizedContent).filter((line) => line.heading?.level === 2).map((line) => line.heading!.title));
  for (const heading of REQUIRED_RESEARCH_SECTIONS) {
    const exists = headings.has(heading);
    if (!exists) {
      issue(heading, "research.heading_missing", `Research needs ${heading} content.`, `Include a ${heading} section with actual research; prose, bullets and tables are all accepted.`);
      continue;
    }
    const section = extractResearchMarkdownSection(visible, heading);
    if (!hasSubstantiveResearchSection(section)) issue(heading, "research.section_non_substantive",
  ...
  if (!containsSourceEvidence(sources) && !hasStructuredSourceEvidence(normalizedContent)) {
    ...
    else issue("Sources", "research.sources_missing", "Research needs a concrete source reference or an explicit evidence gap with blocked recommendations.",
  ...
  // Structured evidence rows enable objective consistency checks without imposing a table format.
  ...
  for (const row of findingRows) {
    ...
    if (/^HIGH$/i.test(row.confidence?.trim() ?? "") && /^(?:unsupported|not_enough_evidence|inferred|inferred_from_supported)$/i.test(status.trim())) {
      issue("Findings", "research.high_confidence_unsupported", "A HIGH-confidence finding is explicitly unsupported or inferred.", "Keep the evidence limitation and lower confidence, or provide direct support.");
    }
    for (const id of refs) {
      if (!sourceIds.has(id)) issue("Sources", "research.source_reference_missing", `Finding references missing source ${id}.`, "Correct the reference or include its observed source.");
    }
    for (const id of splitResearchTypedReferenceIds(row.evidence_ids || "")) {
      if (!sourceIds.has(id) && !evidenceIds.has(id)) issue("Sources", "research.source_reference_missing", `Finding references missing evidence ${id}.`, "Correct the reference or include its observed evidence.");
    }
  }
  for (const row of recommendationRows) {
    const refs = splitResearchTypedReferenceIds(row.supporting_claim_ids || row.claim_ids || "");
    for (const id of refs) {
      if (!findingIds.has(id)) issue("Recommendations", "research.finding_reference_missing", `Recommendation references missing finding ${id}.`, "Correct the reference or include the finding.");
      const finding = findingRows.find((item) => (item.finding_id || item.claim_id) === id);
      if (/^ready$/i.test(row.status?.trim() ?? "") && finding && /^(?:unsupported|not_enough_evidence)$/i.test((finding.support_status || finding.claim_class || "").trim())) issue("Recommendations", "research.recommendation_unsupported", "A ready recommendation relies on an explicitly unsupported finding.", "Mark the recommendation blocked or supply supporting evidence.");
    }
    for (const id of splitResearchTypedReferenceIds(row.evidence_ids || "")) {
      if (!sourceIds.has(id) && !evidenceIds.has(id)) issue("Recommendations", "research.source_reference_missing", `Recommendation references missing evidence ${id}.`, "Correct the reference or include its observed evidence.");
    }
  }
...
function validateCodebaseArtifactContent(content: string, artifactId: ArtifactContractId) {
  return validateCodebaseContent(content, artifactId);
}
...
type UiSpecAuthoringMode = "contract" | "skip" | "unknown";

function detectUiSpecAuthoringMode(
  content: string,
  ...
  if (contractHeadings.some((heading) => extractMarkdownSection(content, heading).trim().length > 0)) {
    return "contract";
  }
...
function phaseArtifactRepairInstruction(args: {
  ...
  if (args.artifact === "ui-spec") {
    if (args.uiSpecMode === "contract") {
      return args.heading
        ? `Populate ## ${args.heading} with substantive UI-contract detail, then retry blueprint_phase_artifact_write. Outcome Mode already selects UI contract, so continue repairing the contract.`
        : "Read the phase.ui-spec contract, repair the missing UI-contract sections for the current mode, then retry blueprint_phase_artifact_write.";
    }

    if (args.uiSpecMode === "skip") {
      return args.heading
        ? `Populate ## ${args.heading} with the explicit UI skip rationale details required for skip mode, then retry blueprint_phase_artifact_write.`
        : "Repair the explicit UI skip rationale sections for the current mode, then retry blueprint_phase_artifact_write.";
    }

    return args.heading
      ? `Choose the intended UI-spec mode first, then retry blueprint_phase_artifact_write: either populate ## ${args.heading} as part of a full UI contract, or replace the artifact with a valid explicit UI skip rationale.`
    ...
    if (extractMarkdownSection(content, "Outcome Mode").trim().length === 0) {
      ...
      diagnostics.push(
        phaseArtifactDiagnostic({
          ...
          message: issue,
          repair:
            "Populate ## Outcome Mode with `- Explicit skip rationale`, then retry blueprint_phase_artifact_write.",
          uiSpecMode,
    ...
    if (extractMarkdownSection(content, "Rationale").trim().length === 0) {
      ...
      diagnostics.push(
        phaseArtifactDiagnostic({
          ...
          message: issue,
          repair:
            "Populate ## Rationale with the explicit UI skip rationale, any safety-gate implication, and a revisit trigger, then retry blueprint_phase_artifact_write.",
          uiSpecMode,
          heading: "Rationale"
        })
      );
  ...
  if (artifact === "ui-spec" && missingRequiredSections.includes("Outcome Mode")) {
    ...
    diagnostics.push(
        phaseArtifactDiagnostic({
          ...
          repair:
            uiSpecMode === "contract"
              ? "Populate ## Outcome Mode with `- UI contract`, then retry blueprint_phase_artifact_write."
              : "Populate ## Outcome Mode with either `- UI contract` or `- Explicit skip rationale`, then retry blueprint_phase_artifact_write.",
          uiSpecMode,
          heading: "Outcome Mode"
        })
      );
...
export async function inspectBlueprintArtifacts(projectRoot: string): Promise<{
  ...
  const workflowArtifactFiles = blueprintFiles.filter(
    (artifact) => !isOperationalOnlyBlueprintArtifact(artifact)
  );
...
export async function blueprintArtifactMutateIndex(
  ...
  if (action === "list") {
    return blueprintArtifactMutateIndexUnlocked(args);
  }
  ...
  if (
    ...
    return blueprintArtifactMutateIndexUnlocked({ ...args, cwd: projectRoot });
  }

  return withBlueprintRepoLock(
    projectRoot,
    `capture-index-${blueprintRepoLockNameSegment(args.target)}`,
    () => blueprintArtifactMutateIndexUnlocked({ ...args, cwd: projectRoot })
  );
}

async function blueprintArtifactMutateIndexUnlocked(
  args: ArtifactMutateIndexArgs
): Promise<ArtifactMutateIndexResult> {
  const projectRoot = await ensureRepoRoot(args.cwd);
  const inspection = await inspectBlueprintArtifacts(projectRoot);
...
async function buildArtifactDigestSections(
  ...
  for (const artifactPath of artifactPaths) {
    ...
    await assertCodebasePublicationComplete(projectRoot, artifactPath);
    const raw = await fs.readFile(absolutePath, "utf8");
  ...
  return args.issues.map((issue) => {
    ...
    return mcpWriteDiagnostic({
      ...
      repair: heading
        ...
        : `Repair ${args.artifactId} against its artifact contract; read blueprint_artifact_contract_read for guidance and retry through the MCP write tool.`,
      retryable: true
    });
  });
}
...
function collectQuickRunLifecycleClaimEntries(
  model: Record<string, unknown>
): Array<{ path: string; value: string }> {
  ...
  const classification = asJsonObject(model.classification);
  ...
  const validation = asJsonObject(model.validation);
  ...
  const repairAttempt = asJsonObject(validation?.repairAttempt);
...
function parseAddTestsReportPhase(reportName: string): string | null {
  ...
  return match ? normalizePhaseNumber(match[1].replace("-", ".")) : null;
}

function canonicalAddTestsReportName(phaseNumber: string): string {
  return normalizeReportSlug(`add-tests-${phaseNumber}`);
}
...
function canonicalAuditFixReportName(phaseNumber: string): string {
  return normalizeReportSlug(`audit-fix-${phaseNumber}`);
}
...
function phaseBackedReportPhaseNumber(
  contractId: ArtifactContractId | null,
  reportName: string
): string | null {
  if (contractId === "report.add-tests") {
    return parseAddTestsReportPhase(reportName);
  }

  if (contractId === "report.audit-fix") {
    return parseAuditFixReportPhase(reportName);
  }

  return null;
}

async function capturePhaseBackedReportTopologySnapshot(args: {
  projectRoot: string;
  reportName: string;
  contractId: ArtifactContractId | null;
}): Promise<PhaseTopologySnapshot | null> {
  const phaseNumber = phaseBackedReportPhaseNumber(args.contractId, args.reportName);
  ...
  try {
    return await resolvePhaseTopologySnapshot({
...
async function withFreshPhaseBackedReportTopology<T>(args: {
  projectRoot: string;
  reportName: string;
  contractId: ArtifactContractId | null;
  expected: PhaseTopologySnapshot | null;
  task: () => Promise<T>;
}): Promise<T> {
  ...
  return withBlueprintRepoLock(args.projectRoot, PHASE_TOPOLOGY_LOCK_NAME, async () => {
    let latest: PhaseTopologySnapshot;

    try {
      latest = await resolvePhaseTopologySnapshot({
        cwd: args.projectRoot,
        phase: expected.phaseNumber
      });
    } catch (error) {
      const detail = error instanceof Error ? ` ${error.message}` : "";

      throw new Error(
        `${args.contractId ?? "Artifact report"} write for ${args.reportName} rejected stale phase topology for Phase ${expected.phaseNumber}.${detail}`
      );
    }
    ...
    if (
      ...
      throw new Error(
        formatStalePhaseTopologyMessage({
          operation: `${args.contractId ?? "Artifact report"} write for ${args.reportName}`,
          expected: expected.fingerprint,
          actual: latest.fingerprint
        })
      );
    }
...
async function buildQuickRunAllowedNextActions(): Promise<string[]> {
  const implementedCommands = await getImplementedCommandNames();
...
export async function blueprintArtifactReportAuthoringContext(
  ...
  if (contractId === "report.quick-run") {
    ...
    const schemas = modelContract ? await quickRunReportModelSchemas({ contract }) : null;
    ...
    return {
      ...
      schemaPath: schemas?.schemaPath ?? modelContract?.schemaPath ?? null,
      baseSchema:
        schemas?.baseSchema ??
        (modelContract ? cloneJsonObject(modelContract.jsonSchema) : null),
      taskSchema:
        schemas?.taskSchema ??
        (modelContract ? cloneJsonObject(modelContract.jsonSchema) : null),
      modelOnly: true,
...
function normalizeQuickRunReportModel(model: Record<string, unknown>): QuickRunReportModel | null {
  ...
  const classification = asJsonObject(model.classification);
  const depthUsed = asJsonObject(model.depthUsed);
  const validation = asJsonObject(model.validation);
  const runMetrics = model.runMetrics === undefined ? undefined : asJsonObject(model.runMetrics);
  ...
  const evidenceRead = normalizeRows(model.evidenceRead, (row) =>
    typeof row.kind === "string" &&
    typeof row.ref === "string" &&
    typeof row.why === "string"
      ? {
          kind: row.kind as QuickRunEvidenceKind,
          ref: row.ref.trim(),
          why: row.why.trim()
        }
      : null
  );
  const changesMade = normalizeRows(model.changesMade, (row) =>
  ...
  const validationCommands = normalizeRows(validation.commands, (row) =>
    typeof row.command === "string" && typeof row.result === "string"
      ? {
          command: row.command.trim(),
          result: row.result as QuickRunValidationCommandResult,
          ...(typeof row.durationMs === "number" ? { durationMs: row.durationMs } : {}),
          ...(typeof row.notes === "string" ? { notes: row.notes.trim() } : {})
        }
      : null
  );
  const validationRepairAttempt =
    validation.repairAttempt === undefined
      ? undefined
      : (() => {
          const repairAttempt = asJsonObject(validation.repairAttempt);
          ...
          if (
            !repairAttempt ||
            typeof repairAttempt.attempted !== "boolean" ||
            typeof repairAttempt.outcome !== "string"
          ) {
            return null;
          }
          ...
          return {
            ...
            outcome: repairAttempt.outcome as QuickRunValidationRepairOutcome,
            ...(typeof repairAttempt.note === "string"
              ? { note: repairAttempt.note.trim() }
              : {})
          };
        })();
  const gates = normalizeRows(model.gates, (row) =>
    typeof row.name === "string" && typeof row.status === "string"
      ? {
          name: row.name.trim(),
          status: row.status as QuickRunGateStatus,
  ...
  if (
    typeof classification.route !== "string" ||
    typeof classification.confidence !== "string" ||
    classificationReasons === null ||
    typeof classification.validationBudget !== "string" ||
    typeof depthUsed.discuss !== "boolean" ||
    typeof depthUsed.research !== "boolean" ||
    typeof depthUsed.validation !== "string" ||
    depthSubagents === null ||
    typeof depthUsed.trackerUsed !== "boolean" ||
    evidenceRead === null ||
    changesMade === null ||
    typeof validation.status !== "string" ||
    validationCommands === null ||
    validationRepairAttempt === null ||
    gates === null ||
    risks === null ||
    deferredWork === null
  ) {
  ...
  if (
    runMetrics &&
    ((
      runMetrics.administrativeToolCalls !== undefined &&
  ...
  return {
    ...
    classification: {
      ...
      confidence: classification.confidence as QuickRunConfidence,
      reasons: classificationReasons,
      validationBudget: classification.validationBudget as QuickRunValidationBudget
    },
    depthUsed: {
      discuss: depthUsed.discuss,
      research: depthUsed.research,
      validation: depthUsed.validation as QuickRunValidationDepth,
    ...
    changesMade,
    validation: {
      status: validation.status as QuickRunValidationStatus,
      commands: validationCommands,
      ...(typeof validation.skippedReason === "string"
        ? { skippedReason: validation.skippedReason.trim() }
        : {}),
    ...
    ...(runMetrics
      ? {
          runMetrics: {
            ...(typeof runMetrics.administrativeToolCalls === "number"
              ? { administrativeToolCalls: runMetrics.administrativeToolCalls }
              : {}),
...
function quickRunHasLifecycleCompletionClaim(value: string): boolean {
  return (
    /\b(?:phase|lifecycle)\s+(?:completion|complete|completed)\b/i.test(value) ||
    /\bcompleted\s+(?:phase|lifecycle)\b/i.test(value)
  );
}

function quickRunSkippedReasonIsSubstantive(value: string | undefined): boolean {
  if (value === undefined) {
    return false;
  }
  ...
  return normalized.length >= 12 && !hasModelPlaceholderLanguage(normalized) && !isGenericNoneValue(normalized);
}

function quickRunRepairAttemptNoteIsSubstantive(value: string | undefined): boolean {
  if (value === undefined) {
    return false;
  }
...
function quickRunNeedsValidationForChangePath(pathValue: string): boolean {
...
function quickRunReportResidualDiagnostics(args: {
  model: Record<string, unknown>;
  normalizedModel: QuickRunReportModel | null;
}): ArtifactReportDiagnostic[] {
  const diagnostics: ArtifactReportDiagnostic[] = [];
  const classification = asJsonObject(args.model.classification);
  const depthUsed = asJsonObject(args.model.depthUsed);
  const validation = asJsonObject(args.model.validation);
  ...
  if (
    !depthUsed ||
    !Array.isArray(depthUsed.subagents) ||
    !depthUsed.subagents.every((item) => typeof item === "string")
  ) {
    diagnostics.push(
      artifactReportDiagnostic({
        source: "residual",
  ...
  const skippedReason = normalizedModel?.validation.skippedReason ??
    (typeof validation?.skippedReason === "string" ? validation.skippedReason.trim() : undefined);
  const repairAttempt = normalizedModel?.validation.repairAttempt ??
    (() => {
      const rawRepairAttempt = asJsonObject(validation?.repairAttempt);

      if (
        !rawRepairAttempt ||
        typeof rawRepairAttempt.attempted !== "boolean" ||
        typeof rawRepairAttempt.outcome !== "string"
      ) {
        return undefined;
      }
      ...
      return {
        ...
        outcome: rawRepairAttempt.outcome as QuickRunValidationRepairOutcome,
        ...(typeof rawRepairAttempt.note === "string"
          ? { note: rawRepairAttempt.note.trim() }
          : {})
      };
    })();
  const validationCommands =
    normalizedModel?.validation.commands ??
  ...
  if (validationStatus === "failed" && !repairAttempt) {
    diagnostics.push(
      artifactReportDiagnostic({
        ...
        message:
          "Quick-run validation.status=failed requires validation.repairAttempt so the report records whether one bounded repair attempt was not attempted or still failing.",
        context: { status: validationStatus },
        suggestion:
          "Set validation.repairAttempt with attempted=false and outcome=not-attempted, or attempted=true and outcome=still-failing."
      })
    );
  }
  ...
  if (repairAttempt) {
    if (!repairAttempt.attempted && repairAttempt.outcome !== "not-attempted") {
      diagnostics.push(
        artifactReportDiagnostic({
          ...
          message:
            "Quick-run validation.repairAttempt.outcome must be not-attempted when attempted is false.",
          context: repairAttempt,
          suggestion:
            "Set repairAttempt.outcome to not-attempted, or set attempted=true for repaired or still-failing outcomes."
        })
      );
    }
    ...
    if (repairAttempt.attempted && repairAttempt.outcome === "not-attempted") {
      diagnostics.push(
        artifactReportDiagnostic({
          ...
          message:
            "Quick-run validation.repairAttempt.outcome cannot be not-attempted when attempted is true.",
          context: repairAttempt,
    ...
    if (validationStatus === "skipped") {
      diagnostics.push(
        artifactReportDiagnostic({
          ...
          message:
            "Quick-run validation.repairAttempt should be omitted when validation.status is skipped.",
          context: { status: validationStatus },
          suggestion:
            "Remove validation.repairAttempt for skipped validation; use validation.skippedReason instead."
        })
      );
    }
    ...
    if (validationStatus === "passed" && repairAttempt.outcome !== "repaired") {
      diagnostics.push(
        artifactReportDiagnostic({
          ...
          message:
            "Quick-run validation.status=passed can only keep validation.repairAttempt when the bounded repair attempt outcome is repaired.",
    ...
    if (validationStatus === "failed" && repairAttempt.outcome === "repaired") {
      diagnostics.push(
        artifactReportDiagnostic({
          ...
          message:
            "Quick-run validation.status=failed cannot use validation.repairAttempt.outcome=repaired.",
          context: { status: validationStatus, repairAttempt },
    ...
    if (repairAttempt.note !== undefined && !quickRunRepairAttemptNoteIsSubstantive(repairAttempt.note)) {
      diagnostics.push(
        artifactReportDiagnostic({
          source: "residual",
          path: "model.validation.repairAttempt.note",
          ...
          message:
            "Quick-run validation.repairAttempt.note must be substantive when provided.",
          context: { repairAttempt },
          suggestion:
            "Use a brief concrete note that explains the bounded repair attempt decision or outcome."
        })
      );
    }
  }

  const codeChanges = normalizedModel?.changesMade.filter((row) =>
    quickRunNeedsValidationForChangePath(row.path)
  ) ?? [];
  if (
    codeChanges.length > 0 &&
    validationStatus === "skipped" &&
    !quickRunSkippedReasonIsSubstantive(skippedReason)
  ) {
    diagnostics.push(
      artifactReportDiagnostic({
        source: "residual",
  ...
  if (
    !normalizedModel?.gates.some((gate) =>
      /overwrite|quick-run-report|quick-report/i.test(gate.name)
    )
  ) {
    diagnostics.push(
      artifactReportDiagnostic({
...
- Reasons:
${renderBulletList(model.classification.reasons).replace(/^/gm, "  ")}
...
- Subagents:
${renderBulletList(model.depthUsed.subagents).replace(/^/gm, "  ")}
...
${model.validation.skippedReason ? `- Skipped Reason: ${model.validation.skippedReason}\n` : ""}${repairAttemptSummary}${renderMarkdownTable(
  ...
  model.validation.commands.map((row) => [
    row.command,
    row.result,
    row.durationMs === undefined ? "none" : String(row.durationMs),
    row.notes ?? "none"
  ])
)}
  ...
  if (!modelObject) {
    diagnostics.push(
      artifactReportDiagnostic({
        ...
        message:
          contractId === "report.quick-run"
            ? "Quick-run report model must be a JSON object."
            : contractId === "report.audit-fix"
...
export async function blueprintArtifactReportWrite(
  ...
  if (
    ...
    return blueprintArtifactReportWriteUnlocked({ ...args, cwd: projectRoot });
  }

  return withBlueprintRepoLock(
    projectRoot,
    `artifact-report-${blueprintRepoLockNameSegment(args.reportName)}`,
    () => blueprintArtifactReportWriteUnlocked({ ...args, cwd: projectRoot })
  );
}

async function blueprintArtifactReportWriteUnlocked(
  args: ArtifactReportWriteArgs
): Promise<ArtifactReportWriteResult> {
  const projectRoot = await ensureRepoRoot(args.cwd);
  const inspection = await inspectBlueprintArtifacts(projectRoot);
  ...
  if (hasModel) {
    ...
    const phaseTopologySnapshot = await capturePhaseBackedReportTopologySnapshot({
      projectRoot,
      reportName: args.reportName,
      contractId
    });
    ...
    return withFreshPhaseBackedReportTopology({
      projectRoot,
      reportName: args.reportName,
      contractId,
...
async function writeCodebaseArtifact(args: ArtifactCodebaseWriteArgs): Promise<ArtifactCodebaseWriteResult> {
  const projectRoot = await ensureRepoRoot(args.cwd);
  const inspection = await inspectBlueprintArtifacts(projectRoot);
  ...
  if (exists) {
    const existingContent = await fs.readFile(absolutePath, "utf8");
    const pristineScaffold = existingContent === renderArtifactScaffoldTemplate(args.artifactId);
    ...
    if (pristineScaffold && !(args.overwrite ?? false)) {
      warnings.push(
        "Replacing the pristine generated scaffold with authored content."
      );
