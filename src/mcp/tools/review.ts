...
type ReviewValidateModelArgs = {
  cwd?: string;
  phase?: NumericInput;
  artifact?: "code-review" | "review-fix" | "security" | "ui-review";
  files?: string[];
  depth?: ReviewDepth;
  targetIds?: string[];
  model: unknown;
...
const reviewValidateModelInputSchema = {
  cwd: z.string().optional(),
  phase: numericBlueprintInputSchema.optional(),
  artifact: z.enum(["code-review", "review-fix", "security", "ui-review"]).optional(),
  files: z.array(z.string()).optional(),
  depth: z.enum(["quick", "standard", "deep"]).optional(),
  targetIds: z.array(z.string()).optional(),
  model: z.unknown()
...
const reviewAuthoringContextInputSchema = {
  cwd: z.string().optional(),
  phase: numericBlueprintInputSchema.optional(),
  artifact: z.enum(["code-review", "review-fix", "security", "ui-review"]),
  files: z.array(z.string()).optional(),
  depth: z.enum(["quick", "standard", "deep"]).optional(),
  targetIds: z.array(z.string()).optional()
};
  ...
  if (!modelObject) {
    diagnostics.push(
      modelDiagnostic({
        ...
        path: "model",
        code: "schema.type",
        message: `${artifact === "security" ? "Security review" : artifact === "review-fix" ? "Review-fix" : artifact === "ui-review" ? "UI-review" : "Code-review"} model must be a JSON object.`,
    ...
    if (!schemaValid) {
      diagnostics.push(
        ...(validate.errors ?? []).map((error) =>
          schemaDiagnosticFromAjvError(error, artifact)
        )
      );
    }
...
export async function blueprintReviewRecord(
  ...
  if (modelOnlyArtifact) {
    const modelArtifact = args.artifact as "code-review" | "review-fix" | "security" | "ui-review";
...
export const reviewToolDefinitions = [
  ...
  },
  {
    ...
    description:
      "Validate a model-authored review.code-review, review.review-fix, review.security, or review.ui-review JSON payload against the runtime task schema, residual quality checks, and canonical Markdown render before persistence.",
  ...
  },
  {
    ...
    description:
      "Build the model-only review authoring context and narrowed task schema for review.code-review, review.review-fix, review.security, or review.ui-review before model drafting.",
  ...
  },
  {
    ...
    description:
      "Persist a phase-scoped Blueprint review artifact with overwrite protection; code-review, review-fix, security, and ui-review persist model-authored JSON only after validator replay.",
