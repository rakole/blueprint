...
test("artifact contract registry exposes canonical contract ids and templates", async () => {
  ...
  assert.match(researchContract.authoringTemplate, /Optional topics may be added when useful/);
  assert.doesNotMatch(researchContract.authoringTemplate, /must use|Keep the canonical ## headings exactly/);
  assert.equal(researchContract.sectionValidations?.["Open Questions"]?.exactEmptySentinel, undefined);
  ...
  assert.deepEqual(researchContract.requiredHeadings, ["Summary", "Recommendations", "Sources"]);
  assert.match(researchContract.authoringTemplate, /## Summary/);
  assert.match(researchContract.authoringTemplate, /## Recommendations/);
  assert.match(researchContract.authoringTemplate, /## Sources/);
  ...
  const contextAuthoringTemplate = renderArtifactAuthoringTemplate("phase.context");
  assert.equal("authoringTemplate" in contextContract, false);
  assert.notEqual(contextAuthoringTemplate, contextContract.scaffoldTemplate);
  ...
  assert.match(contextAuthoringTemplate, /## Discovery Grounding/);
  assert.match(
  ...
  assert.doesNotMatch(contextAuthoringTemplate, /Project brief:/);
  assert.doesNotMatch(contextAuthoringTemplate, /Requirements grounding:/);
  assert.doesNotMatch(contextAuthoringTemplate, /Workflow posture:/);
  assert.doesNotMatch(contextAuthoringTemplate, /Confirmed decisions:/);
  assert.doesNotMatch(
    contextAuthoringTemplate,
    /\*Generated by `blueprint_artifact_scaffold`\*/
  );
  assert.match(contextAuthoringTemplate, /## Implementation Decisions/);
  assert.doesNotMatch(contextAuthoringTemplate, /<implementation decision 1>/);
  assert.match(contextAuthoringTemplate, /## Specific Ideas/);
  assert.doesNotMatch(contextAuthoringTemplate, /<specific idea 1>/);
  assert.match(contextAuthoringTemplate, /## Existing Code Insights/);
  assert.doesNotMatch(contextAuthoringTemplate, /<existing code insight 1>/);
  ...
  assert.match(contextAuthoringTemplate, /## Canonical References/);
...
test("research contract accepts absent optional requirement tables without filler", () => {
  const research = canonicalResearchContent("Keep registry updates aligned with MCP.", "");
...
## Locked Decisions From Context`
  ...
  assert.match(validation.issues.join("\n"), /section Summary needs actual findings/i);
...
test("research contract does not reject optional question formatting", () => {
  const research = canonicalResearchContent(
    "Publish useful research without requiring an empty-question sentinel.",
    "| LIFE-01 | Keep endpoint research grounded. | Optional metadata does not block publication. |"
  ).replace(/## Open Questions\s+[\s\S]*?\n## Confidence Breakdown/, "## Open Questions\n\n- null\n\n## Confidence Breakdown");
  const validation = validateResearchArtifactContent(research);
  assert.equal(validation.valid, true, validation.issues.join("\n"));
});
...
test("research accepts meaningful aliases and unconstrained optional heading names", () => {
  const research = canonicalResearchContent(
    "Preserve the findings without rejecting equivalent section wording.",
    "| LIFE-01 | Keep endpoint research grounded. | Formatting is not an implementation decision. |"
  ).replace("## Don't Hand-Roll", "## Do Not Hand Roll")
    .replace("## Summary", "## Executive Summary")
    .replace("## Recommendations", "## Recommended Approach")
...
test("research contract keeps dependency/tool guidance authoring-time without validator warnings", () => {
  ...
  assert.match(authoringTemplate, /Optional topics may be added when useful/i);
  assert.doesNotMatch(authoringTemplate, /### Dependency \/ Tool Evaluation/i);
  assert.equal(validation.valid, true, validation.issues.join("\n"));
