...
test("repo-facing status docs treat new-workspace as a shipped Wave 5 command", async () => {
  ...
  assert.match(architectureFile, /shipped Wave 5 maintenance commands, `new-workspace`, `cleanup`, and `reapply-patches`/i);
  assert.match(handoffFile, /shipped Wave 5 maintenance commands `new-workspace`, `cleanup`, and `reapply-patches`/i);
  assert.match(
    progressFile,
