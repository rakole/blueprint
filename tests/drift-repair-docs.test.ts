...
test("control-plane docs describe the shipped Wave 0 runtime instead of a docs-only state", async () => {
  ...
  assert.match(agents, /Phase 2\.2 future-contract drift-repair checkpoint/);
  ...
  assert.match(handoff, /current checkpoint is Phase 2\.2 future-contract drift repair/i);
  assert.match(memory, /Current milestone: Phase 2\.2 future-contract drift repair/);
  ...
  assert.match(state, /active checkpoint/);
...
test("drift requirements are backfilled and phase-mapped across roadmap and state docs", async () => {
  ...
  assert.match(requirements, /\| DRIFT-07 \| Phase 2\.2 \| Pending \|/);
  ...
  assert.match(state, /Phase 2\.1 closed on 2026-04-11; Phase 2\.2 is the active checkpoint/);
});
