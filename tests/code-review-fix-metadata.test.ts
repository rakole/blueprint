...
test("code-review-fix manifest references findings tools, canonical contracts, and safe follow-up routing", async () => {
  ...
  assert.match(commandFile, /default the remediation candidate set to saved `follow-up` findings only/i);
  assert.match(commandFile, /selected saved target ids and evidence packet/i);
  assert.match(commandFile, /`fix`, `defer`, or `skip` reclassification/i);
  assert.match(commandFile, /process one selected target at a time/i);
  assert.match(commandFile, /Author a `review\.review-fix` JSON model only/i);
  assert.match(commandFile, /camelCase fields such as `remediationSummary`, `findingsAddressed`, `changesMade`/i);
  assert.match(commandFile, /Literal rendered heading keys like `Remediation Summary`/i);
  assert.match(commandFile, /same `targetIds` array used for authoring context/i);
  assert.match(commandFile, /`base: "synced"`\s+and an explicit patch/i);
  assert.match(commandFile, /activeCommand: "\/blu-code-review-fix"/i);
...
test("blueprint-review skill captures review-fix rules on top of the saved findings substrate", async () => {
  ...
  assert.match(skillFile, /camelCase fields/);
  assert.match(skillFile, /Literal rendered headings or\s+locked-marker labels are forbidden as JSON keys/i);
...
test("code-review-fix local runtime contract locks richer saved-finding remediation behavior", async () => {
  ...
  assert.match(referenceFile, /default the candidate\s+remediation set to saved `follow-up` findings only/i);
  ...
  assert.match(referenceFile, /Decide `fix`, `defer`, or `skip`/i);
  assert.match(referenceFile, /stale-evidence notes/i);
  ...
  assert.match(referenceFile, /base: "synced"/i);
  assert.match(referenceFile, /activeCommand: "\/blu-code-review-fix"/i);
...
test("blueprint-reviewer agent stays code-review-focused while remaining safe for explicit reuse", async () => {
  ...
  assert.match(agentFile, /selected saved target ids/i);
  assert.match(agentFile, /`fix`, `defer`, or `skip`/);
  assert.match(agentFile, /staleEvidence/);
  assert.match(agentFile, /Do not act as a fixer or executor agent/i);
  assert.match(agentFile, /browser-only\s+analysis/i);
  assert.doesNotMatch(agentFile, /\/blu-audit-fix|selected finding ids or summaries/i);
});
