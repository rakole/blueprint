...
test("maintenance skill captures pr-branch filtering, report persistence, and source-branch safety", async () => {
  ...
  assert.match(skillFile, /Maintenance\s+commands do not currently declare optional agents/i);
  assert.match(skillFile, /same-named Gemini CLI agent tool/i);
  assert.match(skillFile, /tool is available in the\s+current host session/i);
  assert.match(skillFile, /Do not read, inline, or load any separate agent source/i);
...
test("pr-branch runtime contract locks commit classification, fallback, and report repair behavior", async () => {
  ...
  assert.match(runtimeContract, /Current default path: no optional agent/i);
  assert.match(runtimeContract, /same-named Gemini CLI agent tool with a bounded review-branch risk packet/i);
  assert.match(runtimeContract, /tool is available in the current host session/i);
  assert.match(runtimeContract, /Do not read, inline, or load any separate agent source before delegation/i);
  ...
  assert.doesNotMatch(runtimeContract, /agent definition/i);
  assert.doesNotMatch(runtimeContract, /agents\/blueprint-/i);
