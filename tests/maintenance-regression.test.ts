...
test("maintenance manifests keep dirty-tree stops, advisory mode gates, and runtime-owned report gates explicit", async () => {
  ...
  assert.match(ship, /`workflow\.code_review=false`/);
  assert.match(ship, /never make security evidence mandatory regardless of `workflow\.secure_phase`/);
  assert.match(ship, /`workflow\.code_review=true` and `workflow\.secure_phase=false`/);
  assert.match(ship, /review evidence may still be mandatory while security evidence is not/);
  assert.match(ship, /require code-review evidence first and secure-phase or security evidence after that before ready shipping/);
...
test("maintenance skill keeps family-wide preflight, pending-gate, and runtime-owned report boundaries aligned", async () => {
  ...
  assert.match(skill, /`workflow\.code_review=false`/);
  assert.match(skill, /security evidence is never mandatory regardless of `workflow\.secure_phase`/);
  assert.match(skill, /`workflow\.code_review=true` and `workflow\.secure_phase=false`/);
  assert.match(skill, /review evidence may still be mandatory while security evidence is not/);
