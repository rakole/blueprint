...
- Current milestone: post-shipment lifecycle and roadmap-admin closeout now also includes the shipped Wave 3 capture slice, the shipped Wave 3 lightweight execution slice, and the shipped Wave 4 docs and review slices; `complete-milestone`, `milestone-summary`, `new-milestone`, `insert-phase`, `note`, `add-todo`, `add-backlog`, `review-backlog`, `quick`, `docs-update`, and `secure-phase` are implemented alongside the earlier lifecycle, governance, and roadmap-admin surfaces
- Runtime status: Wave 0 plus the Phase 3 discovery commands (`discuss-phase`, `research-phase`, `ui-phase`), the roadmap-discovery command `list-phase-assumptions`, the lifecycle commands `plan-phase`, `execute-phase`, `validate-phase`, `verify-work`, the Wave 3 lightweight execution command `quick`, the router command `next`, the governance handoff/resume commands `pause-work` and `resume-work`, the Wave 2 roadmap-admin commands `add-phase`, `insert-phase`, `remove-phase`, `plan-milestone-gaps`, `audit-milestone`, `complete-milestone`, `milestone-summary`, and `new-milestone`, the Wave 3 capture commands `note`, `add-todo`, `add-backlog`, and `review-backlog`, the Wave 4 docs command `docs-update`, and the Wave 4 review command `secure-phase` are implemented, and routing still filters to implemented commands only
...
- Implementation strategy: keep the shipped Wave 2 closeout plus `insert-phase`, `note`, `add-todo`, `add-backlog`, `review-backlog`, `quick`, `docs-update`, and `secure-phase` contracts locked, preserve the closed Phase 2.2 and shipped Phase 3 guarantees, and only expand routing when manifests, primary skills, and required MCP tools all line up
...
- keep command-catalog rollout aligned with each shipped Wave 3 and Phase 4 command, including `note`, `add-todo`, `add-backlog`, `review-backlog`, `quick`, `docs-update`, and `secure-phase`
...
- Future sessions should treat the Wave 2 milestone-closeout trio plus `insert-phase`, `note`, `add-todo`, `add-backlog`, `review-backlog`, `quick`, `docs-update`, and `secure-phase` as shipped, and only start a new slice once its manifest, primary skill, and required MCP tools are planned together
