...
- keep command-catalog rollout aligned with each shipped Wave 3 and Phase 4 command, including `note`, `add-backlog`, `docs-update`, and `secure-phase`
...
- Future sessions should treat the Wave 2 milestone-closeout trio plus `note`, `add-backlog`, `docs-update`, and `secure-phase` as shipped, keep `insert-phase` blocked until its own substrate exists, and only start a new slice once its manifest, primary skill, and required MCP tools are planned together
