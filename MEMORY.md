...
- Current milestone: post-shipment lifecycle and roadmap-admin closeout now includes the shipped Wave 2 milestone-closeout trio plus the first shipped Wave 4 docs command; `complete-milestone`, `milestone-summary`, `new-milestone`, and `docs-update` are implemented alongside the earlier lifecycle, governance, and roadmap-admin surfaces, while `insert-phase` remains blocked
- Runtime status: Wave 0 plus the Phase 3 discovery commands (`discuss-phase`, `research-phase`, `ui-phase`), the roadmap-discovery command `list-phase-assumptions`, the lifecycle commands `plan-phase`, `execute-phase`, `validate-phase`, `verify-work`, the router command `next`, the governance handoff/resume commands `pause-work` and `resume-work`, the current Wave 2 roadmap-admin commands `add-phase`, `remove-phase`, `plan-milestone-gaps`, `audit-milestone`, `complete-milestone`, `milestone-summary`, and `new-milestone`, and the Wave 4 docs command `docs-update` are implemented, and routing still filters to implemented commands only
