...
- For the active GSD milestone, start with Phase 1: Bug Taxonomy And Reporting Harness. Define the `docs/bugs/` index, bug-report template, severity/confidence vocabulary, affected-surface taxonomy, and no-fix boundary checks before auditing workflow-specific slices.
...
- `plan-phase` now uses `blueprint_phase_plan_index`, `blueprint_phase_plan_read`, `blueprint_phase_plan_authoring_context`, `blueprint_phase_plan_validate_model`, and `blueprint_phase_plan_write` to persist real `XX-YY-PLAN.md` content from structured `phase.plan` models validated against a runtime-narrowed JSON Schema before write
- `validate-phase` now uses `blueprint_phase_summary_index`, `blueprint_phase_summary_read`, `blueprint_phase_validation_read`, `blueprint_phase_validation_render`, and `blueprint_phase_validation_write` to persist real `XX-VERIFICATION.md` content; `phase.verification` and `phase.uat` also expose structured `modelContract` metadata, and validation writes accept exactly one of rendered Markdown `content` or a structured model rendered and validated by MCP
