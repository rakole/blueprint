...
- `validate-phase` now uses `blueprint_phase_summary_index`, `blueprint_phase_summary_read`, `blueprint_phase_validation_read`, `blueprint_phase_validation_render`, and `blueprint_phase_validation_write` to persist real `XX-VERIFICATION.md` content; `phase.verification` and `phase.uat` also expose structured `modelContract` metadata, validation writes accept exactly one of rendered Markdown `content` or a structured model rendered and validated by MCP, and `phase.verification` 1.1.0 preserves status/gate parity, COVERED coverage normalization, text-visible schemas/diagnostics/evidence for Tabnine-style hosts, and extended validation details instead of requiring models to drop them
