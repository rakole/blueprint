...
- continue `/blu-impact` from `docs/IMPACT-WORKFLOW-IMPLEMENTATION-PLAN.md` Phase 7 next; Phase 6 added typed obligation records, command-substrate checks, planned-command exposure review gates, context uncertainty handling, and dist/build provenance while keeping the command planned and non-routable because the manifest and primary skill are still intentionally absent
