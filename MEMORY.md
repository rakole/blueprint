...
- keep `new-workspace` and `remove-workspace` aligned with their host-global registry contract, transactional workspace writes or removals, and implemented-only routing guarantees
...
- `remove-workspace` shipped on 2026-04-23 with a dedicated `/blu-remove-workspace` manifest, the `blueprint-maintenance` skill, the new `blueprint_workspace_remove` MCP tool, exact host-global registry resolution through `blueprint_workspace_registry_get`, confirmation-gated workspace previews, dirty-repo and registry-drift blockers, and safe worktree or clone teardown before the matching registry entry is removed
...
- Future sessions should treat the Wave 2 milestone-closeout trio plus `insert-phase`, `note`, `add-todo`, `check-todos`, `add-backlog`, `review-backlog`, `fast`, `quick`, `debug`, `docs-update`, `code-review`, `audit-fix`, `secure-phase`, `review`, `ui-review`, `add-tests`, `pr-branch`, `ship`, `remove-workspace`, `cleanup`, and `reapply-patches` as shipped, and only start a new slice once its manifest, primary skill, and required MCP tools are planned together
