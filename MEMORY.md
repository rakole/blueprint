...
- Current milestone: post-shipment lifecycle and roadmap-admin closeout now also includes the shipped Wave 3 capture slice plus the shipped Wave 4 docs and review slices; `complete-milestone`, `milestone-summary`, `new-milestone`, `insert-phase`, `add-backlog`, `docs-update`, and `secure-phase` are implemented alongside the earlier lifecycle, governance, and roadmap-admin surfaces
...
- Implementation strategy: keep the shipped Wave 2 closeout plus `insert-phase`, `add-backlog`, `docs-update`, and `secure-phase` contracts locked, preserve the closed Phase 2.2 and shipped Phase 3 guarantees, and only expand routing when manifests, primary skills, and required MCP tools all line up
...
- Future sessions should treat the Wave 2 milestone-closeout trio plus `insert-phase`, `add-backlog`, `docs-update`, and `secure-phase` as shipped, and only start a new slice once its manifest, primary skill, and required MCP tools are planned together
