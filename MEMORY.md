...
- Current milestone: post-shipment lifecycle and roadmap-admin closeout now also includes the shipped Wave 3 capture slice, the shipped Wave 3 lightweight execution slice, the shipped Wave 3 debug slice, and the shipped Wave 4 docs, review, and review-branch slices; `complete-milestone`, `milestone-summary`, `new-milestone`, `insert-phase`, `note`, `add-todo`, `check-todos`, `add-backlog`, `review-backlog`, `fast`, `quick`, `debug`, `docs-update`, `code-review`, `secure-phase`, and `pr-branch` are implemented alongside the earlier lifecycle, governance, and roadmap-admin surfaces
- Runtime status: Wave 0 plus the Phase 3 discovery commands (`discuss-phase`, `research-phase`, `ui-phase`), the roadmap-discovery command `list-phase-assumptions`, the lifecycle commands `plan-phase`, `execute-phase`, `validate-phase`, `verify-work`, the Wave 3 lightweight execution commands `fast` and `quick`, the Wave 3 debug command `debug`, the router command `next`, the governance handoff/resume commands `pause-work` and `resume-work`, the Wave 2 roadmap-admin commands `add-phase`, `insert-phase`, `remove-phase`, `plan-milestone-gaps`, `audit-milestone`, `complete-milestone`, `milestone-summary`, and `new-milestone`, the Wave 3 capture commands `note`, `add-todo`, `check-todos`, `add-backlog`, and `review-backlog`, the Wave 4 docs command `docs-update`, the Wave 4 review commands `code-review` and `secure-phase`, and the Wave 4 shipping command `pr-branch` are implemented, and routing still filters to implemented commands only
...
- Implementation strategy: keep the shipped Wave 2 closeout plus `insert-phase`, `note`, `add-todo`, `add-backlog`, `review-backlog`, `fast`, `quick`, `debug`, `docs-update`, `code-review`, `secure-phase`, and `pr-branch` contracts locked, preserve the closed Phase 2.2 and shipped Phase 3 guarantees, and only expand routing when manifests, primary skills, and required MCP tools all line up
...
- keep the shipped `complete-milestone`, `milestone-summary`, `new-milestone`, and `pr-branch` contracts aligned with their docs, manifests, shared skills, and implemented-only routing behavior
...
- keep command-catalog rollout aligned with each shipped Wave 3 and Phase 4 command, including `note`, `add-todo`, `check-todos`, `add-backlog`, `review-backlog`, `fast`, `quick`, `debug`, `docs-update`, `code-review`, `secure-phase`, and `pr-branch`
...
- `debug` shipped on 2026-04-13 with dedicated `/blu-debug` and `/blu:debug` manifests, the `blueprint-debug` skill, the `blueprint-debugger` agent contract, a canonical `.blueprint/reports/debug-latest.md` report, and optional todo capture through `blueprint_artifact_mutate_index`
- `code-review` shipped on 2026-04-13 with dedicated `/blu-code-review` and `/blu:code-review` manifests, the discoverable `blueprint-review` skill, the bounded `blueprint-reviewer` agent contract, deterministic scope resolution through `blueprint_review_scope`, and review persistence through `blueprint_review_record`
...
- `pr-branch` shipped on 2026-04-13 with dedicated `/blu-pr-branch` and `/blu:pr-branch` manifests, the `blueprint-maintenance` skill, a canonical `.blueprint/reports/pr-branch-latest.md` report, and explicit git confirmation gates grounded in `blueprint_project_status`, `blueprint_config_get`, and `blueprint_artifact_summary_digest`
- Future sessions should treat the Wave 2 milestone-closeout trio plus `insert-phase`, `note`, `add-todo`, `check-todos`, `add-backlog`, `review-backlog`, `fast`, `quick`, `debug`, `docs-update`, `code-review`, `secure-phase`, and `pr-branch` as shipped, and only start a new slice once its manifest, primary skill, and required MCP tools are planned together
