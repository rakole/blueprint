...
- Current milestone: post-shipment lifecycle and roadmap-admin closeout now includes the shipped Wave 2 milestone-closeout trio plus `insert-phase` and the first shipped Wave 4 docs command; `complete-milestone`, `milestone-summary`, `new-milestone`, `insert-phase`, and `docs-update` are implemented alongside the earlier lifecycle, governance, and roadmap-admin surfaces
- Runtime status: Wave 0 plus the Phase 3 discovery commands (`discuss-phase`, `research-phase`, `ui-phase`), the roadmap-discovery command `list-phase-assumptions`, the lifecycle commands `plan-phase`, `execute-phase`, `validate-phase`, `verify-work`, the router command `next`, the governance handoff/resume commands `pause-work` and `resume-work`, the current Wave 2 roadmap-admin commands `add-phase`, `insert-phase`, `remove-phase`, `plan-milestone-gaps`, `audit-milestone`, `complete-milestone`, `milestone-summary`, and `new-milestone`, and the Wave 4 docs command `docs-update` are implemented, and routing still filters to implemented commands only
...
- Implementation strategy: keep Wave 2 closeout contracts locked, preserve the closed Phase 2.2 and shipped Phase 3 guarantees, keep the shipped `insert-phase` slice aligned with its MCP substrate and regression coverage, and only expand routing when manifests, primary skills, and required MCP tools all line up
...
- keep command-catalog rollout aligned with each shipped Phase 4 command, starting with `docs-update`
...
- Future sessions should treat the Wave 2 milestone-closeout trio plus `insert-phase` and `docs-update` as shipped, and only start a new slice once its manifest, primary skill, and required MCP tools are planned together
