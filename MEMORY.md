...
- Total retained commands: 53, plus the explicitly approved additive `/blu-impact` command while it is implemented behind non-routable catalog gating
...
- continue `/blu-impact` from `docs/IMPACT-WORKFLOW-IMPLEMENTATION-PLAN.md` Phase 8 next; Phase 7 added deterministic `PASS`/`WARN`/`BLOCK` scoring, separate risk and confidence modeling, first-class scope unknowns, stable report ordering, and normalized report data while keeping the command planned and non-routable because the manifest and primary skill are still intentionally absent
