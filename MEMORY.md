...
- Implementation strategy: keep the shipped Wave 2 closeout plus `insert-phase`, `note`, `add-todo`, `add-backlog`, `review-backlog`, `fast`, `quick`, `docs-update`, and `secure-phase` contracts locked, preserve the closed Phase 2.2 and shipped Phase 3 guarantees, and only expand routing when manifests, primary skills, and required MCP tools all line up
