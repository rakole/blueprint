...
export function renderCommandCatalogMarkdown(registry: GeneratedCommandRegistry): string {
  const implemented = registry.commands.filter((entry) => entry.implemented).sort(sortCommands);
  const nonRunnable = registry.commands.filter((entry) => !entry.implemented).sort(sortCommands);
  const lines = [
    "# Blueprint Command Catalog",
    "",
    `This file is generated by ${inlineCode(GENERATED_BY)} from ${inlineCode(SOURCE_METADATA)} via ${inlineCode(REGISTRY_PATH)}. Do not hand-edit command rows; run ${inlineCode("npm run generate:commands")} after command metadata changes.`,
    "",
  ...
  for (const command of implemented) {
    lines.push(
      [
        inlineCode(command.command),
        String(command.wave),
        escapeMarkdownTableCell(command.family),
        inlineCode(command.primarySkill),
        escapeMarkdownTableCell(command.executionProfile ?? "unknown"),
        markdownList(command.writes),
    ...
    for (const command of nonRunnable) {
      lines.push(
        [
          ...
          inlineCode(command.status),
          inlineCode(command.declaredStatus),
          command.blockedBy.length > 0
            ? escapeMarkdownTableCell(command.blockedBy.join("<br>"))
            : "none",
          escapeMarkdownTableCell(command.risk)
        ].join(" | ").replace(/^/, "| ").replace(/$/, " |")
      );
...
export function renderRuntimeReferenceMatrix(registry: GeneratedCommandRegistry): string {
  const implemented = registry.commands.filter((entry) => entry.implemented).sort(sortCommands);
  const nonRunnable = registry.commands.filter((entry) => !entry.implemented).sort(sortCommands);
  const lines = [
    RUNTIME_MATRIX_START,
    `This command matrix is generated by ${inlineCode(GENERATED_BY)} from ${inlineCode(REGISTRY_PATH)}. Runtime metadata remains canonical in ${inlineCode(SOURCE_METADATA)}; this snapshot is reviewable documentation, not a second status authority.`,
    "",
    "### Root Router",
    "",
    ...
    "|---|---|---|---|---|---|---|---|",
    [
      inlineCode(registry.rootRouter.command),
      inlineCode(registry.rootRouter.manifestPath),
      inlineCode(registry.rootRouter.primarySkill),
      markdownList(registry.rootRouter.requiredTools),
      "none",
      "none",
      escapeMarkdownTableCell(registry.rootRouter.contractNotes),
      inlineCode("source-owned")
    ].join(" | ").replace(/^/, "| ").replace(/$/, " |"),
    "",
  ...
  for (const command of implemented) {
    const runtimeReference = command.runtimeReference;

    lines.push(
      [
        inlineCode(command.name),
        inlineCode(command.sourceId ?? command.specPath ?? "unknown"),
        inlineCode(command.primarySkill),
        markdownList(command.requiredTools),
        markdownList(command.optionalAgents),
        command.runtimeReference?.hookInvolvement.length
          ? escapeMarkdownTableCell(command.runtimeReference.hookInvolvement.join("; "))
          : "none",
        escapeMarkdownTableCell(runtimeReference?.contractNotes ?? command.purpose ?? ""),
        markdownList(runtimeReference?.evidenceState ?? [])
      ].join(" | ").replace(/^/, "| ").replace(/$/, " |")
    );
  }
...
export async function renderUpdatedRuntimeReference(
  ...
  return replaceHeadingSection({
    content: currentRuntimeReference,
    heading: "## Command Runtime Matrix",
    nextHeading: "## Hook Surface Matrix",
    replacementBody: surfaces.runtimeReferenceMatrixBlock
  });
}
