...
- active phase and routing command
- blockers
...
`STATE.md` is a compact routing/status artifact, not plan memory or a general knowledge base. Detailed phase plans, summaries, decisions, handoffs, and accumulated context stay in their owning phase or report artifacts.
...
- `next_action`
- `last_updated`
- `progress.total_phases`, `progress.completed_phases`, and `progress.percent`, derived from roadmap phase completion

The metadata frontmatter is generated by the state MCP tools and is disposable: stale frontmatter must not override the Markdown body or runtime artifact inspection when loading, updating, syncing, or routing state.
...
- should be normalized through the `phase.uat` model contract plus narrowed runtime `taskSchema`, using `blueprint_phase_validation_validate_model.renderPreview` as the canonical preview before persistence
