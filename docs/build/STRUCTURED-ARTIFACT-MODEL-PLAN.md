...
- 2026-04-29: `phase.verification` moved its base model shape into `src/mcp/artifact-contracts/schemas/phase.verification.model.schema.json`; `blueprint_phase_validation_authoring_context` now returns a narrowed task schema for completed summary paths and allowed next actions, and `/blu-validate-phase` validates the model before model-only persistence.
