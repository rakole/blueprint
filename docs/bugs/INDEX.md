...
| BPBUG-006 | New-project catalog status ignored bundled bootstrap runtime inputs | medium | confirmed | command | fixed | 10 | `new-project` runtime metadata now requires the three bundled bootstrap reference inputs, and focused coverage proves a missing input downgrades the command out of implemented resource exposure. | The thin `new-project` manifest and skill input bundle moved bootstrap behavior into local reference files, but `NEW_PROJECT_RUNTIME_METADATA.requiredInputPaths` did not list those files for catalog substrate checks. | [BPBUG-006](./BPBUG-006-new-project-runtime-inputs-untracked.md) |
...
| cross-layer contract synchronization gaps | BPBUG-006 | Runtime-owned command metadata did not declare all local runtime inputs required by the corresponding manifest and skill input bundle. | Keep runtime metadata required input paths aligned with manifest and skill-package inputs, and add catalog/resource regressions for missing bundled inputs. |
...
| BPBUG-006 | Fixed | Repaired in `codex/new-project-runtime-inputs`; retained here as the completed new-project substrate-gating item. | Missing bundled bootstrap reference inputs now block implemented catalog/resource exposure for `new-project`. | `npm run typecheck`, `npx tsx --test tests/new-project-metadata.test.ts` `4/4`, and `npm run build --silent` |
...
| BPBUG-006 | fixed | Repair branch `codex/new-project-runtime-inputs`; verification `npm run typecheck`, `npx tsx --test tests/new-project-metadata.test.ts` `4/4`, and `npm run build --silent` |
...
| Phase 10 | command-runtime-metadata vs manifest/skill input drift for `new-project`; implemented-only catalog/resource substrate gating for missing bundled bootstrap references | broad control-plane docs drift and command-catalog prose parity | BPBUG-006 |
