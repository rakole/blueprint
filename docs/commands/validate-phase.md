...
- Keep the saved-summary-first contract explicit throughout the run: execution summaries are the validation baseline, overwrite confirmation is the pending gate when an existing `XX-VERIFICATION.md` would change, and the next safe action stays on `/blu-validate-phase <phase>` only until the saved verification artifact is either ready for the configured PASS route or truthfully routes to an implemented repair command such as `/blu-add-tests <phase>` for saved `deferred-test` evidence. With `workflow.no_uat=false`, PASS routes to `/blu-verify-work <phase>`; with `workflow.no_uat=true`, PASS routes to `/blu-progress` while `/blu-verify-work` remains manual.
...
- Apply the pre-write routing shorthand from `skills/blueprint-phase-validation/references/validate-phase-runtime-contract.md` before the final verification model validation and write; keep the branch details canonical in that runtime contract and enforced by the narrowed task schema plus route diagnostics.
...
- Only route the next safe action to the configured PASS route when the saved artifact says `Gate State: PASS`, readiness is ready for UAT, and no unresolved gap or repair signals remain: `/blu-verify-work <phase>` when `workflow.no_uat=false`, `/blu-progress` when `workflow.no_uat=true`. When the saved artifact contains explicit test-generation gaps such as `deferred-test`, route to `/blu-add-tests <phase>`; when implementation or behavior gaps remain, route to `/blu-audit-fix <phase>` instead of coercing the gate to PASS or looping back through validation.
