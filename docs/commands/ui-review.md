...
- Route next safe action explicitly: prefer `/blu-validate-phase <phase>` when `XX-VERIFICATION.md` is still missing, otherwise prefer `/blu-verify-work <phase>` when `XX-UAT.md` is still missing and `workflow.no_uat=false`, and otherwise prefer `/blu-progress`. When `workflow.no_uat=true`, missing UAT should not route UI-review back to `/blu-verify-work`; explicit `/blu-verify-work` remains manual.
