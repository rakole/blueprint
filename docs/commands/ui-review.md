...
- Route next safe action explicitly: prefer `/blu-validate-phase <phase>` when `XX-VERIFICATION.md` is still missing, otherwise keep saved invalid/`FAIL`/`PARTIAL`/incomplete UAT on the saved implemented repair action or `/blu-verify-work <phase>`, then prefer `/blu-verify-work <phase>` when `XX-UAT.md` is still missing and `workflow.no_uat=false`, and otherwise prefer `/blu-progress`. When `workflow.no_uat=true`, only missing UAT should route UI-review to `/blu-progress`; saved invalid, `FAIL`, `PARTIAL`, or incomplete UAT should stay on the saved implemented repair action or `/blu-verify-work <phase>`, and explicit `/blu-verify-work` remains manual.
