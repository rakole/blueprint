...
- `blueprint_phase_plan_authoring_context` -> `{status, phase, planId, path, schemaPath, baseSchema, taskSchema, knownRequirements, knownEvidenceArtifacts, allowedDependencyPlanIds, planningReadiness}`
...
- Treat `blueprint_phase_plan_authoring_context.status` as the safe-to-draft signal for interpreting `taskSchema`, not as a plain schema-availability flag. Invalid results can still include `baseSchema`, `taskSchema`, known requirements, known evidence artifacts, allowed dependency plan ids, and `planningReadiness` for inspection while upstream readiness is blocked.
...
- `blueprint_phase_plan_validate_model` runs best-effort rendered-preview preflight internally before trimming its public response, so standalone validation can surface prospective dependency, cycle, and wave-order drift without exposing the large preview/schema payloads; roadmap coverage remains warning-level during incremental authoring.
- Treat `blueprint_phase_plan_write.validation.valid === true` as "this incremental write is acceptable to save now", not as proof that the full saved phase plan set is complete. Successful incremental writes can still return top-level `warnings` and `validation.warnings` saying final plan-set validation remains invalid until uncovered roadmap requirements are handled.
...
- Final roadmap coverage for `blueprint_phase_plan_validate` continues to come from the saved plan frontmatter `requirements` arrays, not from `Requirement Coverage` table rows.
