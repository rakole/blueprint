...
- When saved execution evidence includes reviewable repo/source files and effective config requires post-UAT gates, completed UAT routes through `/blu-code-review <phase>` first and only routes through `/blu-secure-phase <phase>` when `workflow.secure_phase=true`.
...
- canonical UAT contract metadata and structured model authority through `blueprint_artifact_contract_read`
...
- `blueprint_artifact_contract_read` -> `{id, canonicalName, scaffoldTemplate, requiredHeadings, lockedMarkers, freehandPolicy, notes, modelContract}`
...
- New or updated UAT artifacts must preserve the current test, test matrix, result counts, blocked prerequisites, structured gaps, and follow-up fix candidates from the model-backed contract plus validation preview before the artifact can count as completion evidence.
...
- After completed UAT, if saved summaries include reviewable repo/source files, route in this order: when `workflow.code_review=true` and no `XX-REVIEW.md` exists -> `/blu-code-review <phase>`; when `workflow.code_review=true` and `workflow.secure_phase=true`, `XX-REVIEW.md` exists, and no `XX-SECURITY.md` exists -> `/blu-secure-phase <phase>`; once any configured security gate is satisfied or not required, open review findings -> the next safe action saved in `XX-REVIEW.md`; configured gates complete -> normal advancement.
- If `workflow.code_review` is false, preserve the older post-UAT advancement behavior regardless of `workflow.secure_phase`. If saved execution evidence has no reviewable repo/source files, preserve the older post-UAT advancement behavior as well. `/blu-secure-phase` remains manually runnable either way.
