...
- Validate the authored JSON through `blueprint_review_validate_model` before persistence, passing `scopeSource` with explicit files so validation proves the later record call has the same provenance. Repair all schema and residual diagnostics together; do not switch to Markdown fallback.
