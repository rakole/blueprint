...
| `blueprint_phase_research_status` | Report discovery readiness for context, research, and UI-spec artifacts, including present-versus-usable validation signals | `{hasContext, hasResearch, hasUiSpec, hasUsableContext, hasUsableResearch, hasUsableUiSpec, contextValid, contextIssues, researchValid, researchIssues, uiSpecValid, uiSpecIssues, suggestedRepairs, warnings}` |
...
- `validate-phase` and `verify-work` use summary index/read, validation read/write, `blueprint_artifact_contract_read`, config, artifact validation, and state update tools so their required tool shapes stay contract-derived. `verify-work` must load `skills/blueprint-phase-validation/references/verify-work-runtime-contract.md`, derive a concrete user-observable UAT test queue from saved summaries plus ready verification evidence, preserve response classification and structured gaps in the canonical UAT artifact, and use the no-subagent fallback when a suitable verifier is unavailable.
- `validate-phase` must load `skills/blueprint-phase-validation/references/validate-phase-runtime-contract.md` as the rich behavior contract, classify State A/B/C before writing, build a requirement/task coverage map with evidence metadata and gap classes, use a capability-gated `blueprint-verifier` when available, preserve the sequential no-subagent fallback when it is not, reject browser/web-search/shell-only or generic agents as substitutes, and repair invalid writes or validation failures once through MCP before treating the workflow as complete.
...
- `code-review` uses `blueprint_phase_locate`, `blueprint_config_get`, `blueprint_artifact_list`, `blueprint_artifact_contract_read`, `blueprint_review_scope`, and `blueprint_review_record` to honor review settings, derive a deterministic repo-file scope from executed plans or explicit file paths, author against `skills/blueprint-review/references/code-review-runtime-contract.md`, and persist `XX-REVIEW.md` against the canonical review contract.
