...
The impact tool family is registered for the planned additive `/blu-impact` command. Phase 5 implements deterministic config loading, scope resolution, read-only context loading, multi-label file-surface classification, ownership coverage, and dependency coverage while keeping report writing and output rendering on safe placeholder seams until later phases land. `blueprint_impact_report_write` remains guarded from writing.
