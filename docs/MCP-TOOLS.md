...
The impact tool family is registered for the planned additive `/blu-impact` command. Phase 7 implements deterministic config loading, scope resolution, read-only context loading, multi-label file-surface classification, ownership coverage, dependency coverage, command-substrate checks, planned-command exposure review gates, path-level review/docs/test/release/migration/security/deployment/build obligations, dist/build provenance checks, final advisory scoring, and a normalized report model. Report writing and output rendering remain on safe placeholder seams until Phase 8 lands, so `blueprint_impact_report_write` remains guarded from writing.
