...
- `execute-phase` uses `blueprint_phase_execution_targets` together with plan index/read, `blueprint_artifact_contract_read` for the phase.summary contract before summary authoring, summary index/read/write, config, artifact validation, and state update tools. Keep the richer execution sequencing, summary-backed checkpointing, and no-report contract in `skills/blueprint-phase-execution/references/execute-phase-runtime-contract.md`; this MCP note stays limited to the deterministic tool surface and its locked invariants.
