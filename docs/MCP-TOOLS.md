...
The impact tool family is registered for the planned additive `/blu-impact` command. Phase 4 implements deterministic config loading, scope resolution, read-only context loading, and multi-label file-surface classification while keeping report writing and output rendering on safe placeholder seams until later phases land. `blueprint_impact_report_write` remains guarded from writing.
...
| `blueprint_impact_analyze` | Normalize files from top-level `changedFiles`, top-level `files`, nested `scope.files`, and nested `scope.changedFiles`; warn on mismatches; classify each file into deterministic multi-label surfaces plus area and surface summaries while keeping advisory `WARN` until later analysis phases land | `{phaseStatus, impactId, status, impactStatus, risk, confidence, surfaces, areaSummary, surfaceSummary, findings, obligations, unknowns, evidence, report, warnings}` |
