...
- `execute-phase` uses plan index/read, `blueprint_artifact_contract_read` for the phase.summary contract before summary authoring, summary index/read/write, config, artifact validation, and state update tools. It preserves wave ordering, blocks later-wave execution while lower-wave pending plans remain, uses bounded executor agents only for disjoint write ownership, falls back to one-plan-at-a-time inline execution when agents are unavailable or unsafe, runs targeted verification plus bounded repair before `COMPLETED` summaries, treats `PARTIAL` and `BLOCKED` summaries as carry-forward evidence rather than completion, and reruns the summary index before updating state.
