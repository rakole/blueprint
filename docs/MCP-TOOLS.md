...
| `blueprint_artifact_mutate_index` | Append or update canonical capture entries in Blueprint indexes such as backlog, notes, and todos, with duplicate detection and optional backlog stub reservation metadata | `{targetPath, createdEntryIds, updatedCounts, duplicateEntryIds, reservedPhase, warnings}` |
