...
The impact tool family is registered for the planned additive `/blu-impact` command. Phase 6 implements deterministic config loading, scope resolution, read-only context loading, multi-label file-surface classification, ownership coverage, dependency coverage, command-substrate checks, planned-command exposure review gates, path-level review/docs/test/release/migration/security/deployment/build obligations, and dist/build provenance checks while keeping report writing and output rendering on safe placeholder seams until later phases land. `blueprint_impact_report_write` remains guarded from writing.
...
| `blueprint_impact_analyze` | Normalize files from top-level `changedFiles`, top-level `files`, nested `scope.files`, and nested `scope.changedFiles`; warn on mismatches; classify each file into deterministic multi-label surfaces; resolve effective impact config from built-ins plus invocation config; analyze CODEOWNERS and optional ownership metadata, fallback reviewers, sensitive missing-owner BLOCKs, package/workspace/import/custom dependency graph coverage, reverse-dependency unknowns, generated/source mixed-scope notes, implemented-command substrate gaps, conservative planned-command exposure review gates, path-level obligations, and dist/build provenance while keeping only Phase 7 full scoring deferred | `{phaseStatus, impactId, status, impactStatus, risk, confidence, surfaces, areaSummary, surfaceSummary, ownership, dependencyGraph, findings, obligations, unknowns, evidence, report, warnings}` |
