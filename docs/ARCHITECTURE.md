...
This includes the shipped Wave 5 maintenance commands `cleanup`, `reapply-patches`, and `workstreams`, together with the newly shipped `new-workspace` flow and the advisory `update` flow.
...
- extra MCP families for workspace removal
...
- It currently registers project/catalog, config, state/pause-handoff, phase/roadmap including backlog promotion, capture-index, artifact, review, workspace, workstream, update, and patch tool families.
- It owns `.blueprint/` reads and writes, config normalization, capture index persistence, phase artifact persistence, summary and validation persistence, milestone audit report writes, state synchronization, the host-global workspace registry plus workspace bootstrap writes, host-global advisory update metadata writes, and host-global patch registry plus audit writes.
...
- Planned tool families for workspace removal remain future contracts until they are registered.
