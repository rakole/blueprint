...
This includes the shipped Wave 5 maintenance commands `new-workspace`, `remove-workspace`, `workstreams`, `cleanup`, and `reapply-patches`.
...
- extra MCP families for update flows
...
- Planned tool families for update behavior remain future contracts until they are registered.
