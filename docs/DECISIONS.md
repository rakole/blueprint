...
3. `BP-003` Install model
   Blueprint is installed as a Gemini CLI or Tabnine CLI extension from GitHub, not by a custom npm installer.
...
10. `BP-010` Global state boundary
    Global mutable state is allowed only under `~/.<host>/blueprint/` and only for non-project concerns such as workspace registry, update metadata, and patch registry.
...
15. `BP-015` No extension self-mutation
    Blueprint commands do not rewrite the installed extension from inside the active host CLI session.

16. `BP-016` `update` behavior
    `/blu-update` is an advisor/checklist command because extension updates happen outside the active host CLI session.

17. `BP-017` `reapply-patches` behavior
    `/blu-reapply-patches` uses a global patch registry in `~/.<host>/blueprint/patches/`; it does not patch the installed extension copy directly.

18. `BP-018` Workspace strategy
    `new-workspace` and `remove-workspace` remain supported without `list-workspaces`. The default workspace root is `~/blueprint-workspaces/<name>`, backed by `~/.<host>/blueprint/workspaces.json`.
...
23. `BP-023` Router behavior
    Router-style commands such as `help`, `progress`, `next`, and `do` are planned as inline host-native routers, not as slash-command chain callers.
...
25. `BP-025` Config layering and defaults
    `.blueprint/config.json` is the canonical repo-level Blueprint config and is stored in normalized full form. Optional user defaults live only in `~/.<host>/blueprint/defaults.json`, with effective config precedence of hardcoded defaults, then user defaults, then repo config, then command flags.
