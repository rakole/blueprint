...
- The maintenance commands `/blu-pr-branch` and `/blu-ship` are now shipped on April 13, 2026, `/blu-undo` is now shipped on April 16, 2026, and `/blu-cleanup` remains shipped; `pr-branch` keeps `.blueprint/` filtering explicit and persists `.blueprint/reports/pr-branch-latest.md`, `ship` reuses saved verification and review evidence and persists `.blueprint/reports/ship-latest.md` with manual fallback guidance when `gh` is unavailable, `undo` previews a safe revert plan and persists `.blueprint/reports/undo-latest.md` before any git revert runs, and `cleanup` archives only completed-milestone phase directories after explicit confirmation and a durable `.blueprint/reports/cleanup-latest.md` plan
