...
- Brownfield mapping writes generated repo context only under `.blueprint/codebase`;
  portable consumers may use the index and selected immutable generation pages.
...
0. Treat `blueprint_phase_context.codebase` as reusable brownfield repo/navigation evidence when it is present. Prefer the compact prepared packet, known live targets, and selected portable routes before re-reading broad repo surfaces. A verified `.blueprint/codebase/INDEX.md` owns its `ENTRY.md` navigation; do not automatically load all compatibility views to enrich the portable map or the full map corpus. Missing, unknown, unsupported, stale, or malformed maps use bounded source discovery or the existing summary path, and are not regenerated by discovery. Treat generated summaries as useful but potentially stale: cite them, then confirm the live repo still agrees before using them as planner-grade truth.
...
2. Read roadmap requirements, refreshed state, prepared codebase/navigation
   evidence, artifact inventory, effective config, any existing spec, and the live
