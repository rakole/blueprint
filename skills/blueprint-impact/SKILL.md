...
6. Use `mcp_blueprint_blueprint_impact_report_write` only when writing is enabled. The writer owns `report.impact` JSON Schema validation, runtime narrowing, residual quality checks, and canonical rendering. Keep writes under `.blueprint/impact/<impact-id>/`, use returned paths as authoritative, and require explicit overwrite confirmation when a changed bundle already exists.
