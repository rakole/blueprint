...
- `blueprint_roadmap_add_phase`: pass the normalized phase-ready description plus confirmed durable `requirementIds`, concrete `goal`, 2-5 item `successCriteria`, and `confirmed: true` after the active command's approval gate. Do not precompute phase numbers, slugs, or phase directories; use the returned `phaseNumber`, `phasePrefix`, and `phaseDir`.
...
8. Use `blueprint_roadmap_add_phase` only when the confirmed target is roadmap-ready active work with confirmed durable requirement IDs, a concrete roadmap objective, 2-5 observable success criteria, and `confirmed: true` from the routing confirmation gate; if project health is partial, route to `/blu-health` before roadmap mutation.
