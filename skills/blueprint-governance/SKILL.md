...
- Treat Blueprint skills as loaded guidance, not callable tools. Governance
  commands do not declare optional agents; keep settings, profile, health,
  pause, and resume flows inline. If a future governance command contract adds
  an optional agent, delegate only by calling the same-named Gemini CLI agent
  tool when the contract permits it, effective config allows optional agents,
  and that same-named tool is available in the current host session. Do not
  read, inline, or load any separate agent source for current governance flows.
